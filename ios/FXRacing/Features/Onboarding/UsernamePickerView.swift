...
struct UsernamePickerView: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .navigationTitle(viewModel.isChange ? "Change Username" : "Username")
            .navigationBarTitleDisplayMode(.inline)
            .toolbar {
                if !viewModel.isChange {
                    // Escape hatch — gives the user a way out if anything in
                    // the onboarding flow misbehaves (server error, network
                    // dead-end, etc). Without this, the only path off this
                    // screen is a successful Confirm, which Apple Review
                    // flagged as a dead-end when it didn't work.
