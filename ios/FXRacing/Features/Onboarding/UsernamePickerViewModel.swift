...
final class UsernamePickerViewModel {
    ...
    func submit(authManager: AuthManager) async -> Bool {
        guard canSubmit else { return false }
        return await performSubmit(authManager: authManager, autoRetriesRemaining: 2)
    }

    private func performSubmit(authManager: AuthManager, autoRetriesRemaining: Int) async -> Bool {
        ...
        do {
            ...
            if isTaken {
                if isAutoGenerated && autoRetriesRemaining > 0 {
                    ...
                    return await performSubmit(authManager: authManager, autoRetriesRemaining: autoRetriesRemaining - 1)
                }
