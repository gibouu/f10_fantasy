...
export async function handleDiagHealthGet(
  req,
  {
    db,
    getNow = () => new Date(),
  ...
  const [upcomingSnapshots, recentSnapshots] = await Promise.all([
    Promise.all(upcoming.map((r) => snapshot(r.id, now, db))),
    Promise.all(recent.map((r) => snapshot(r.id, now, db))),
  ])

  const allIssues = [
    ...upcomingSnapshots.filter((s) => s !== null),
    ...recentSnapshots.filter((s) => s !== null),
  ]
  ...
  return Response.json({
    ...
    upcoming: upcomingSnapshots.filter((s) => s !== null),
    recent: recentSnapshots.filter((s) => s !== null),
    issues: allIssues,
