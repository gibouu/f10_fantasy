...
export default async function getRaceResults({ initial, session_key }: Props) {
    const sessionRes = await fetch(
        `https://api.openf1.org/v1/sessions?session_key=${session_key}&session_name=Race&year=${season}`
    );
    ...
    if (!session || !session.session_key) {
        return { raceResults: [], raceStatus: 'inactive' as SessionStatus };
    }
    ...
    let raceStatus: SessionStatus = (currentTime >= startTime && currentTime <= endTime) ? 'active' : 'inactive';

    // If the session is inactive and it's not an initial fetch, return early with status
    if (raceStatus === 'inactive' && !initial) {
        return { raceResults: [], raceStatus };
    }
    ...
    return { raceResults: latestResults, raceStatus };
}
